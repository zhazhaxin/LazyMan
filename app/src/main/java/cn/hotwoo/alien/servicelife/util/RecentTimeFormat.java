 ...
 */
public class RecentTimeFormat implements TimeFormat.DateFormat {
    ...
    public String otherFormat(TimeFormat timeFormat, long millisecond) {
        ...
        if (mistime > 0) {
            ...
            } else return timeFormat.timeToString(millisecond,"yyyy-MM-dd");
        } else {
            long time = -mistime;
            int s = (int) (time / 1000);
            if (s < 60) {
                ...
                return timeFormat.timeToString(millisecond,"yyyy-MM-dd");
        }
    }
}